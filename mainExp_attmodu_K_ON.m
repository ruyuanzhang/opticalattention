...
for i = 1:nOnTrials
%     if isempty(on.T(i).behavior)
%        error('empty value');
%     end
    correctIdx(i) = on.T(i).behavior(1); % 0,correct; 1, false positive,2, false negative
...
savefig('K_mainExp_posisensi_ON.fig');
...
localizerData = load('localizer_PCA_K.mat');
localizerMaps = localizerData.valueImg_mat_demean;
localizerMask = localizerData.validMask;

PC1 = load('mainExp_PCA_K_ON.mat','PC1');
PC1 = PC1.PC1;
...
savefig(h, 'K_mainExp_denoisedmaindata.fig');
...
axis off;
set(gca,'View',[45 90]);
title('Monkey K, attend in - out');

savefig('K_attend_betadiff_ON.fig');
